...
class LPN(nn.Module):
    ...
    def scalar(self, x):
        """
        Returns f(x) = phi((I - P)x) + 0.5 * ||P x||^2,
        where P projects onto the span of the all-ones vector
        (i.e., the per-sample global mean broadcast to x's shape).

        Input shape: (batch, C, H, W) with H == W.
        ...
        # compute projections P x and (I - P) x
        mu = x.mean(dim=(1, 2, 3), keepdim=True) 
        # (I - P) x
        x_perp = x - mu
        # ||P x||^2 = (sum x)^2 / n
        n = x.shape[1] * x.shape[2] * x.shape[3]
        s = x.view(bsize, -1).sum(dim=1, keepdim=True)
        p_norm_sq = (s ** 2) / float(n)
        ...
        for core, res, sz in zip(self.lin[1:-2], self.res[:-1], size[:-1]):
            x_scaled = nn.functional.interpolate(x_perp, (sz, sz), mode="bilinear")
            y = self.act(core(y) + res(x_scaled))

        x_scaled = nn.functional.interpolate(x_perp, (size[-1], size[-1]), mode="bilinear")
        y = self.lin[-2](y) + self.res[-1](x_scaled)  # 1x1 if input is divisible down to 1
        y = self.act(y)
        ...
        phi_term = y**2 + self.alpha * x_perp.reshape(bsize, -1).pow(2).sum(1, keepdim=True)
        proj_term = 0.5 * p_norm_sq

        f = phi_term + proj_term
        return f
