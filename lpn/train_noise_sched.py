...
def parse_args():
    ...
    parser.add_argument( 
        "--scheduled_noise", action="store_true", help="Scheduled noise levels." ### Added scheduled noise levels
    )
    parser.add_argument(
        "--initial_noise_level", type=float, default=0.5, help="Initial noise level." ### Added initial noise level
    )
    parser.add_argument(
        "--final_noise_level", type=float, default=0.1, help="Final noise level." ### Added final noise level
    )
    parser.add_argument(
        "--noise_schedule_steps", type=int, default=4000, help="Number of steps to decrease noise level."
    )
    parser.add_argument(
        "--decrease_increment", type=float, default=0.1, help="Amount to decrease noise level by."
    )
...
"""
Gets the noise level for the current global steps.
If scheduling is on, uses the scheduled noise level via a linear decrease.
"""
def get_noise(args, global_steps):
    if args.scheduled_noise:
        noise = args.initial_noise_level - (global_steps // args.noise_schedule_steps) * args.decrease_increment
        if noise < args.final_noise_level:
            noise = args.final_noise_level
    else:
        noise = args.sigma_noise
    return noise
...
def main(args):
    ...
    while True:
        for step, batch in enumerate(train_dataloader):
            ...
            # Train step
            noise = get_noise(args, global_step)
            result = train_step(
                model, optimizer, batch, loss_func, noise, device
            )
            loss = result["loss"]
